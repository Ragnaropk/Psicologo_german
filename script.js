...
let activeFile = certificates[0]?.file ?? "";
...
function renderCertificates(items) {
  ...
  if (!items.length) {
    ...
    viewerTitle.textContent = "Sin resultados";
    viewerDescription.textContent = "Prueba con otra palabra clave para volver a ver las constancias disponibles.";
    return;
  }
  ...
  const activeCertificate =
    items.find((certificate) => certificate.file === activeFile) ?? items[0];
